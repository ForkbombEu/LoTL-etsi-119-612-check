...
| TS 119 602 JSON/XML core checks | `src/json/loteChecks.ts`, `src/xml/loteMetadata.ts`, `src/standards/ts119602Entities.ts`, `src/standards/ts119602Syntax.ts` | Marks TS 119 612 as not applicable for JSON LoTE, enforces binding-specific direct entity/service nesting and cardinality, and validates multilingual names, addresses, pointers, URI syntax and local Annex B transliteration evidence. |
