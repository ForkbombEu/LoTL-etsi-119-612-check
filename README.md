...
representations are compared within each service identity. Authoritative
legal/contact claims are checked only when explicit records are supplied;
trusted-entity/service pointed-content encoding and parser capability remain
...
- Archive checks follow bounded same-origin HTML links and JSON fields named `url`, `href`, `location`, `artifact`, `artifactUrl`, or `download`; other archive protocols remain `inconclusive`.
- Annex I register authentication supports compact JWS with an embedded `x5c` signer matching the service identity. Other signature/seal formats and register-specific record schemas remain `inconclusive`.
- Scheme-page meaning and authoritative identity/contact claims require explicit source-identified evidence; legal prose is never interpreted autonomously.
- Finite history-retention policies remain contextual. Final closed lists require caller-supplied profile/scheme URIs whose meaning is exactly `expired`.
